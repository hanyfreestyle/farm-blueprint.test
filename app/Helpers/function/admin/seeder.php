...
if (!function_exists('loadSeederFromClientOrDefSeparate')) {
  function loadSeederFromClientOrDefSeparate(
    string $clientDataName,
    string $defaultDataName,
    ?string $subFolder = null,
    ?string $clientLangName = null,
    ?string $defaultLangName = null,
  ): void {
    $folderName = config('appConfig.client_name') ? config('appConfig.client_name').'/' : '';
    $clientDataPath = public_path('db/'.$folderName.$clientDataName.'.sql');

    if (File::isFile($clientDataPath)) {
      DB::unprepared(file_get_contents($clientDataPath));

      $clientLangFile = $clientLangName ?? $clientDataName.'_lang';
      $clientLangPath = public_path('db/'.$folderName.$clientLangFile.'.sql');

      if (File::isFile($clientLangPath)) {
        DB::unprepared(file_get_contents($clientLangPath));
      }

      return;
    }

    $defaultDataPath = public_path('db/'.$subFolder.$defaultDataName.'.sql');

    if (File::isFile($defaultDataPath)) {
      DB::unprepared(file_get_contents($defaultDataPath));

      $defaultLangFile = $defaultLangName ?? $defaultDataName.'_lang';
      $defaultLangPath = public_path('db/'.$subFolder.$defaultLangFile.'.sql');

      if (File::isFile($defaultLangPath)) {
        DB::unprepared(file_get_contents($defaultLangPath));
      }
    }
  }
}
