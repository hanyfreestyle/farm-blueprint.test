...
### 4.2 الهيكل المستهدف المعتمد للمراجعة — لم ينفذ بعد

تمت الموافقة مبدئيًا على الاتجاه التالي قبل إعادة بناء Seeders الأقسام:
...
**مهم:** لا يتم تعديل Section Seeders إلى هذا الشكل النهائي قبل استكمال مراجعة Subsections للأقسام 3–6 وإنشاء خطة تنفيذ واضحة من ملف:
...
## 7. القسم الثالث المستهدف — بيانات الحيوان وتكوين القطيع

القسم الحالي في الكود ما زال باسم:

`تكوين وإدخال القطيع`

لكن المراجعة المعمارية اتجهت إلى الاسم:

`بيانات الحيوان وتكوين القطيع`

والشجرة المقترحة حاليًا، إلى أن تكتمل مراجعة Workflow، هي:
...
### اتجاهات النقل من الشجرة القديمة

- `الدخول الأول للمزرعة والتقييم الأولي` → يتجه إلى Workflow.
- `تسكين القطيع وإدارة الإشغال` → يتجه إلى Workflow.
- `تحويل إنتاج المزرعة إلى القطيع والإحلال الداخلي` → يتجه إلى Workflow.
- `جاهزية القطيع` لا تبقى كتلة واحدة:
  - قواعد الجاهزية → Settings.
  - تنفيذ القرار أو الانتقال → Workflow.
  - عرض الجاهزية وأسباب عدمها → Reports / Dashboard.

هذه الاتجاهات موثقة بالتفصيل في `QUESTIONNAIRE_SECTION_ARCHITECTURE_REVIEW.md` ولا تنفذ قبل اكتمال مراجعة الأقسام المرتبطة.

---

## 8. اتجاه القسم الرابع — الحركات ودورة التشغيل الفعلية

هذا القسم مسؤول عن **الأحداث والحركات التي تحدث فعليًا بمرور الوقت**.

يجب عند مراجعته التأكد من تغطية المسارات التالية وعدم الاكتفاء بالدورة التناسلية:

- استقبال الحيوان من الخارج.
...
- الحالات الاستثنائية.
- الخروج النهائي.
- تشغيل/صيانة/تطهير مواقع الإيواء عند الحاجة.
...
## 9. اتجاه القسم الخامس — التقارير والتحليلات والتنبيهات ومؤشرات الأداء
